...
2인(songlee=S, hoyabom=HB) 전용 여행 계획 PWA. 브레인스토밍 칸반 보드 → 일자별 타임시트(05시 경계) 드래그 배치 → 지도 동선 → 지출/결산 → Gemini AI 도우미 → 드로우 보드(손그림·스티커·배경 사진·카드 연결·PNG, M52a/M52b). 시놀로지 NAS 자가호스팅 + 자동 동기화.
...
src/
  ...
  draw/                  ← simplify(RDP+양자화)·geometry(도형·맞힘·글자폭)·pages(이름·목록·손댄 때)·limits(용량 경고)·tools(색·굵기·스티커·투명도)·**png(내보내기)** — 순수함수+테스트 (M52a/M52b)
...
5. 완료 기준: `npm run typecheck && npm run build && npm run test && npm run e2e` **전부 그린** 후에만 커밋. 현재 기준선: **단위 1544 / e2e 282** (M52a-fix+M52b — `draw2.spec` 8건 추가, 단위 +55). (e2e는 `npm run preview`가 `dist/`를 그대로 내주므로 **돌리기 전에 반드시 `npm run build`** — 안 하면 옛 화면을 검사한다.)
...
15. **드로우 배경은 페이지당 한 장이고 자리를 옮길 수 없다** (M52b). 넣은 사진은 페이지
    가운데에 비율대로 깔리는 것이 전부다 — 확대해서 일부만 쓰거나, 두 장을 나란히 놓거나,
    사진을 끌어 옮기는 것은 없다. 배경을 「요소의 한 종류」로 바꾸면 전부 따라오지만, 그러면
    배경이 껍데기가 아니라 요소가 되어 병합 규칙이 바뀐다 — 다음 회차의 결정거리다.
