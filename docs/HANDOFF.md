...
5. 완료 기준: `npm run typecheck && npm run build && npm run test && npm run e2e` **전부 그린** 후에만 커밋. 현재 기준선: **단위 1628 / e2e 310** (M53-fix — `draw5.spec` 5건 추가, 단위 +3(`centeredView`). 그 전 M53-2가 1625/305, M53-1이 1590/293, M52b가 1544/282). (e2e는 `npm run preview`가 `dist/`를 그대로 내주므로 **돌리기 전에 반드시 `npm run build`** — 안 하면 옛 화면을 검사한다.)
