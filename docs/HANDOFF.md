...
2인(songlee=S, hoyabom=HB) 전용 여행 계획 PWA. 브레인스토밍 칸반 보드 → 일자별 타임시트(05시 경계) 드래그 배치 → 지도 동선 → 지출/결산 → Gemini AI 도우미 → 드로우 보드(손그림·스티커·사진·컬러 보드·선택/리사이즈·PNG, M52a~M53). 시놀로지 NAS 자가호스팅 + 자동 동기화.
...
src/
  ...
  draw/                  ← simplify(RDP+양자화)·geometry(도형·맞힘·글자폭·스냅·스무딩)·transform(리사이즈·마퀴·복붙)·pages(이름·목록·겹침 순서)·limits(용량 경고)·tools(색 41·굵기·스티커·격자·지우개 크기)·**png(내보내기)** — 순수함수+테스트 (M52a~M53)
...
**용량 가드는 막지 않는다.** `src/draw/limits.ts`의 `DRAW_WARN_BYTES = 8_000_000`(M52b-fix에서 1.5MB→8MB — 서버 `MAX_BODY_BYTES` 10MB의 80%. 100MB급을 원하면 상수가 아니라 저장 위치를 바꿔야 한다: 드로우는 워크스페이스 JSON 안이라 획마다 통째로 PUT된다)
...
### M52a-fix + M52b — 동시 편집의 이름·딥링크, 그리고 배경·카드 연결·PNG (2026-09-05)

6기 검증(스크래치패드 `draw{O1,O2,S1,S2,H1,H2}`)이 확정한 결함 다섯을 고치고, M52a가
일부러 남겨 둔 셋(배경 사진·카드 연결·PNG)을 붙였다. **모델은 여전히 additive**다:
`Card.drawPageId?`가 하나 늘었을 뿐이고 `DrawPage.background`는 M52a에서 이미 있었다.

**①「페이지 이름이 상대의 획에 덮인다」 (drawO2 C5).** 원인은 스토어였다:
`putPage`가 **요소 하나를 그릴 때도** 페이지 껍데기의 `updatedAt`을 찍었고, 병합의
껍데기 LWW가 그 도장으로 제목·배경·`elementOrder`를 통째로 골랐다. 그래서 이름을 바꾼
쪽이 상대의 획 하나에 졌다. 이제 스토어의 손이 둘로 갈린다 — `putPageShell`(제목·배경·
삭제 도장, 시각을 찍는다)과 `putPageBody`(요소 추가·수정·삭제, **찍지 않는다**). 세 가지가
따라온다: (a) 목록의 「수정 시각」은 저장된 값이 아니라 `draw/pages.pageTouchedAt`이
껍데기와 요소의 최댓값으로 **계산**한다. (b) `elementOrder`는 껍데기 LWW의 배열 하나가
아니라 두 배열을 겹친 뒤(`mergeSequences`) 살아남은 요소로 재조정한다 — 그러지 않으면
동시에 그린 두 획 중 하나가 배열에서 빠져 맨 뒤 층으로 다시 붙는다. (c) **지운 뒤에
상대가 그렸으면 페이지가 살아난다**를 병합이 명시적으로 판정한다(요소 하나가 `deletedAt`
보다 늦으면 되살린다). 요소가 껍데기 시각을 밀지 않게 된 순간 그 판정이 껍데기 LWW에서
사라지기 때문이고, 톰스톤이 나중에 편집된 엔티티를 이기지 못하는 규칙과 같은 답이다.

**②「다른 탭에서 딥링크를 밟으면 id가 사라진다」 (drawO2 D4/D6).** `HashSync.applyHash`가
`setTab('draw')`를 **먼저** 부르고 그 뒤에 두 번째 칸을 읽었다 — 그 사이에 구독자가 깨어나
「드로우인데 페이지가 없다」를 보고 주소를 `#/draw`로 고쳐 썼고, 다음 줄이 읽은 것은 그
고쳐진 주소였다. 이제 **해시를 먼저 다 읽고 한 번의 `setState`로** 탭과 페이지를 함께
반영한다. 앱 안에서 페이지를 여는 길도 같은 한 줄이 됐다: `uiStore.openDrawPage(id)` —
카드의 🎨 칩이 그 위에 선다.

**③ 페이지 순서가 상대의 페이지 추가에 덮인다 (C8).** `Trip`은 통째로 LWW되는 엔티티라,
페이지를 하나 붙이며 `trip.updatedAt`을 찍으면 그 도장이 **상대가 손으로 만든 순서**를
이긴다. `addDrawPage`·`duplicateDrawPage`는 이제 `drawPageOrder`만 늘리고 도장은 찍지
않는다(붙인 페이지는 병합의 `reconcileOrder`가 어차피 되찾아 준다). 도장을 찍는 것은
**사람이 순서를 바꾼** `moveDrawPage`뿐이다.

**④ 한글 글자의 맞힘 상자가 62%였다 (drawO1 F-1).** `elementBounds`가 글자 수 × 0.62em을
썼는데 한글은 한 칸을 다 먹는다 — 「오사카 어디 갈까」의 오른쪽 절반이 지우개도 선택도
닿지 않는 죽은 자리였다. 이제 **문자마다** 어림한다(CJK·전각·이모지 1.0em, 그 밖 0.62em,
`[...text]`로 코드포인트가 아니라 문자 단위로). 실측: 선택 테두리 494px vs 실제 글자
480px(예전 298px). 줄바꿈도 같은 함수가 다룬다(가장 긴 줄이 폭, 줄 수가 높이).

**⑤ 오른쪽 버튼으로 그려졌다.** `onPointerDown`에 `event.button !== 0` 가드. `isPrimary`는
**마우스에만** 묻는다 — 두 번째 손가락은 `isPrimary`가 false이고, 그것까지 막으면 핀치
줌이 사라진다.

**⑥ 데스크톱 편의.** 도구 버튼에 `title`(+단축키 숫자), 그리고 단축키: Ctrl/Cmd+Z·
Ctrl/Cmd+Shift+Z·Ctrl+Y·Delete/Backspace·Esc·숫자 1~9. **입력칸에 포커스가 있으면 전부
물러선다**(시트가 열리면 그 입력칸이 포커스를 가져가므로 규칙 하나로 덮인다).

**⑦ 탭을 다녀와도 그대로.** `stores/drawSession.ts`(신규, **비영속 모듈 메모리** —
`stores/memoDraft`와 같은 이유)가 페이지별 뷰(중심·배율)·도구 서랍·실행취소 스택을 든다.
탭을 옮기면 `DrawEditor`는 언마운트되지만 서랍은 남는다. **새로고침은 초기화다 — 의도된
것이다**: 그건 데이터가 아니라 손의 자리이고, 며칠 뒤에 되살아나면 오히려 놀랍다.
뷰 상태에 `pageId`를 달아 두는 이유는 페이지를 바꾼 첫 렌더에서 옛 뷰가 새 페이지의
서랍에 잘못 저장되지 않게 하기 위해서다.

**⑧ 터치 타깃.** `formStyles.TOUCH_ICON_BUTTON_CLASS`(신규 — 모바일 44px / lg 36px,
앱 표준 `BTN_SIZE`와 같은 두 단)를 드로우 도구 바·페이지 카드의 아이콘 버튼에. 실측:
320/360/390px에서 도구 버튼·카드 버튼 전부 44×44, 문서 가로 넘침 0, 도구 바는 탭 바 위.
도구 바가 두 줄 44px가 되면서 폰의 자리비움도 6.5rem → 7.5rem.

**⑨ 엣지 8종.** 페이지 이름은 저장 직전에 잘린다(`clampPageTitle`, `DRAW_TITLE_MAX = 60`;
빈 이름·공백은 원래 이름을 지킨다). 글자 요소는 500자 상한(`DRAW_TEXT_MAX`)이고 줄바꿈은
`tspan` 여럿으로 그려진다(입력칸이 `textarea`가 됐다 — Enter는 넣기, Shift+Enter가 줄바꿈).
실측: 스티커 100개 연타 3.3s·요소 100개, 페이지 50장 목록 가로 넘침 0, 지운 페이지 딥링크는
목록으로, 빈 페이지 복제 정상, 콘솔 오류 0.

**⑩ 배경 사진 (M52b).** ⋯ 메뉴 → 「배경 사진」. 바이트는 카드 사진과 **완전히 같은 길**로
들어간다(`preparePhoto` → `photoBlobs` → `photoSync`), 페이지는 `{photoId, opacity}` 하나만
든다. 그래서 **`utils/photos.referencedPhotoIds`에 드로우 배경을 더하는 세 줄이 이 기능의
목숨줄이다** — 그것이 없으면 `photoGc`가 30초 뒤에 배경을 지운다(e2e가 GC를 두 번 돌려
지켜본다). 사진 포함 백업(`exportImport.referencedIds`)에도 같은 세 줄. 화면에서는 `<image>`
하나가 요소보다 **먼저** 그려지고, 자리는 `preserveAspectRatio="xMidYMid meet"`가 정한다
(페이지 4000×4000 안에 원본 비율로, 가운데). 투명도는 0.2~1(`clampOpacity`) — 0을 허용하지
않는 이유는 완전히 사라진 배경을 되돌릴 손잡이가 화면에 없기 때문이다. 복제한 페이지는
같은 `photoId`를 **공유한다**(블롭이 불변이라 안전하고, 그래서 배경을 뗄 때 바이트를 직접
지우지 않고 GC에 맡긴다).

**⑪ 카드 🎨 연결 (M52b).** `Card.drawPageId?: Id` additive. 방향이 카드 → 페이지인 이유는
카드가 하나를 가리키고 페이지는 여럿에게 가리켜지기 때문이다(반대로 두면 같은 사실이 두
곳에 적힌다). 카드 편집 시트의 「드로우 페이지」 줄에서 고르거나 **「새 페이지 만들어 연결」**
(카드 제목으로 페이지가 그 자리에서 만들어진다 — 사진 한 장이 그런 것과 같은 이유로 저장을
기다리지 않는다). 카드에는 🎨 칩이 **칩 배열 맨 끝**에 서고(📷와 같은 이유로 먼저 접힌다)
탭하면 `openDrawPage`로 건너간다. 페이지 헤더의 「🎨 N」 팝오버는 그 필드를 거꾸로 훑어
만든다. **페이지를 지우면 칩만 조용히 사라지고 카드의 필드는 남는다**(택1·문서화): 삭제
실행취소가 페이지를 되살리면 연결도 함께 돌아와야 하고, 「지웠으니 카드도 고치자」는 병합이
갈린 기기에서 그 카드를 통째로 덮는 편집이 된다. 링크 칸에 앱 자신의 `#/draw/<id>`를 넣으면
`normalizeUrl`이 그대로 두고(예전에는 `https://#/…`가 됐다) 칩이 아니라 그 링크가 앱 안에서
열린다(`isInAppHash`).

**⑫ PNG 내보내기 (M52b).** ⋯ → 「PNG로 저장」. 요소를 다시 그리는 **두 번째 렌더러를 만들지
않는다** — 라이브 `<svg>`를 복제해 `viewBox`만 갈아 끼운다(캔버스는 화면 밖 요소까지 DOM에
들고 있다). 복제본이 손보는 것은 셋: CSS 변수로 칠한 종이를 날 값으로, blob: URL인 배경을
data URI로(둘 다 떨어져 나온 SVG 안에서는 풀리지 않는다), 선택 테두리는 걷어 낸다. 잘라 낼
영역은 `exportBounds`(요소+배경+여백 40, 빈 페이지는 1200×900)이고 배율은 2x. 폰에서는
`navigator.canShare({files})`면 공유 시트, 아니면 내려받기. **컨테이너의 헤드리스 크로미움은
`<a download>`의 비ASCII 파일 이름을 버린다**(실기 크롬·사파리는 한글 이름을 그대로 쓴다) —
그래서 e2e는 이름 규칙만 영문 제목으로 확인하고, 한글 이름은 `draw/png.test.ts`가 지킨다.

**신규 파일** `src/stores/drawSession.ts`·`src/draw/png.ts`·`e2e/draw2.spec.ts`(+각 테스트).
**기존 스펙은 한 건도 고치지 않았다** — 이번 회차의 e2e는 전부 additive다.

패치노트 v24.

### M53-1 — 드로우 편집기 재설계: 선택·리사이즈·다중 선택·겹침·복붙 (2026-09-05)

M53 대개편의 **블록 1**(손과 선택). 모델 변경 **0** — 리사이즈도 다중 선택도 복붙도 전부
기존 필드로 표현된다. 블록 2(이미지 요소·컬러 보드)가 딛고 설 바닥을 먼저 깔았다.

**① 제스처 상태 머신 하나.** M52a는 `draft`(state) + `dragging` + `panFrom` + `gesture`(ref
셋)를 따로 들었다. 거기에 리사이즈와 마퀴가 붙으면 조합이 폭발한다(팬 도중에 리사이즈가
시작되는 따위). 이제 판별 유니온 하나다: `draw|pan|pinch|move|resize|marquee`. 우선순위는
`onPointerDown` **한 곳**에서만 정해지고, 그 맨 위는 여전히 **두 손가락은 언제나 팬/줌**이다
(두 번째 손가락이 닿는 순간 하던 손짓은 없던 일이 된다 — 그리기든 리사이즈든).

**② 실행취소 걸음이 묶음이 됐다.** `DrawStep = { ops: DrawOp[]; order?: {before, after} }`.
`DrawOp` 자신은 한 글자도 안 바뀌었다. 셋을 골라 옮기거나 넷을 붙여넣고 **Ctrl+Z 한 번**이면
돌아간다. `order`가 따로 있는 이유는 겹침 순서가 요소의 필드가 아니라 페이지의 배열이기
때문이다(요소 변화와 순서 변화는 한 걸음 안에 함께 들 수 있고, 서로를 흉내 낼 수는 없다).

**③ 「모르는 요소는 안 보이고 안 맞는다」 default 넷.** `DrawElementView`의 switch,
`elementBounds`(0 상자), `hitTest`(false), `moveElementPatch`(빈 패치), 그리고 새
`resizeElementPatch`(빈 패치). **이 커밋이 블록 2의 전제다**: `image` 요소가 든 워크스페이스가
이 배포를 안 받은 빌드에 닿는 자리를 미리 메운다. 규칙으로 못박는다 — **요소 타입 추가에서
옛 빌드의 최선의 결말은 「그 요소만 안 보이는 것」이다**(M52a가 톰스톤 닫힌 집합에서 피했던
그 함정의 요소판). 그래서 블록 2는 **배포 2단계**다.

> **M53-fix에서 실측으로 완화.** 처음 이 자리에 적었던 「exhaustive switch가 `undefined`를
> 돌려주고 **렌더가 죽는다**」는 과장이었다. 테스터 3기가 M52b 프로덕션 빌드로 `image` 요소가
> 든 페이지를 실제로 열어 봤다: **크래시가 없다**(`pageerror` 0 · 편집기 정상 · 요소 수 0 —
> 리액트가 컴포넌트의 `undefined`를 「아무것도 그리지 않음」으로 받는다). 위험한 것은 렌더가
> 아니라 **요소를 만지는 순간**이다 — `elementBounds`의 `undefined`에서 `.x`를 읽는 자리
> (선택 상자·PNG 경계)와 `hitTest`가 boolean이 아닌 것을 돌려주는 자리. default 넷은 그래서
> 여전히 필요하고, 규칙도 그대로다. 바뀐 것은 **최악의 모양**뿐이다.

**④ 화면 표식 제거의 일반화.** `png.ts`가 걷어 내던 `[data-testid="draw-selection"]` 하나가
`[data-draw-chrome]` **속성 규칙**이 됐다. 선택 테두리·리사이즈 핸들·마퀴가 스스로 그 속성을
달고, 다음에 무엇이 늘어도 파일에서 자동으로 빠진다.

**⑤ 선택/리사이즈 (`src/draw/transform.ts`, 신규·순수).** `resizeElementPatch(el, from, to)`가
전부다: 획은 점 배열을 아핀 스케일하되 **`width`는 스케일하지 않는다**(굵기는 그림이 아니라
`hitTest`의 자이자 형광펜 배수의 근거다 — 왕복하면 정수 반올림으로 원래 굵기가 안 돌아온다),
도형·선은 좌표를 매핑, 글자·스티커는 `size`만 균등 스케일. 핸들은 요소의 `<g>` 안이 아니라
**캔버스 오버레이**에 그린다(`DrawElementView`는 아무 이벤트도 듣지 않는다는 규칙을 지키려고),
맞힘도 SVG가 아니라 순수 함수 `pickHandle`이다 — 크기 10/scale, 여유 24/scale(`eraserPad`와
같은 배율 나누기). **여유는 상자의 짧은 변의 3분의 1을 넘지 않는다**: 그 단서가 없으면 48px짜리
스티커는 네 모서리의 여유가 한가운데에서 만나 「옮길 수 없는 물건」이 된다. 부호 뒤집힘은
`normalizeBox`가 흡수하고 최소 크기는 8px(원래 두께가 0인 축은 0으로 남긴다 — 가로선의 높이에
8을 강제하면 선이 이유 없이 내려앉는다). 저장은 여전히 `endPointer` **한 번**이다.

**⑥ 다중 선택 (#1).** `selectedIds: Set`. **`select` 도구에서 빈 곳을 끌면 마퀴**이고 다른
도구는 예전 그대로다(전 도구에 적용하면 손 도구의 팬이 죽는다 — M51의 교훈대로 데스크톱에서는
재현되지 않는 종류의 회귀다). 마퀴는 「완전히 품은 것」이 아니라 **스치기만 해도** 잡는다(폰에서
획 하나를 통째로 감싸려면 페이지를 축소해야 한다). Shift+클릭은 토글이고 이동을 시작하지
않는다. 이미 고른 것을 짚으면 묶음 전체가 따라오고, 묶음 리사이즈는 묶음 bbox를 `from`/`to`로
준 같은 함수 하나가 답한다. 글자·스티커가 하나라도 섞이면 핸들이 **모서리 넷(균등)**으로 준다.

**⑦ 겹침 순서 (#6).** 선택 팝오버의 「맨앞·앞으로·뒤로·맨뒤」. 순수 함수는
`draw/pages.reorderIds`(고른 것끼리는 서로를 밀지 않고, **보이지 않는 이웃은 자리를 세지
않는다** — 지운 요소와 자리를 바꾸면 눈에는 아무 일도 안 일어난다), 스토어는
`setDrawElementOrder`(신규)가 `putPageBody`로 쓴다 — **껍데기 도장은 찍지 않는다**(M52a-fix ①).
**동시 편집에서는 늦게 저장한 쪽이 이긴다**: `mergeSequences`는 두 배열의 상대 순서를 이어
붙일 뿐이라 재배열을 확정적으로 보존하지 않는다. 요소를 잃지는 않는다(모르는 id는 버리고 빠진
id는 뒤에 붙인다).

**⑧ 키보드·접근성 (#8).** 화살표 1px / Shift+화살표 10px, Ctrl/Cmd+C·V·D, Delete는 이제
**고른 것 전부**를 한 걸음으로 지운다. 선택 상태는 `aria-live` 한 줄(`draw-selection-status`)이
읽어 준다 — 점선 테두리는 읽어 주지 않는다.

**⑨ 클립보드 (B1).** `drawSession`의 **모듈 메모리**(OS 클립보드가 아니다: 우리 JSON을 거기
넣으면 다른 앱에 붙는 쓰레기가 되고, `navigator.clipboard.read()`는 모바일 웹뷰에서 자주 없다).
**페이지 밖에 살아서 「페이지 간 복사」가 공짜로 따라온다.** 붙여넣기는 새 id·`+16` 계단
오프셋(연타하면 한 칸씩 더 내려간다)·**한 걸음**이고, 붙인 것이 곧바로 선택되어 손에 잡힌다.
폰에는 Ctrl이 없으므로 팝오버의 「복제」 버튼이 같은 일을 한다 — **그 한 줄이 모바일 동등성이다**.

**⑩ 곁다리 둘.** 선택한 글자를 **더블탭하면 그 자리에서 고친다**(같은 시트를 값과 함께 다시
연다 — 글자를 다 지우고 넣으면 그건 「지운다」는 뜻이다). 그리고 색·굵기는 규칙 하나다:
**고른 게 없으면 다음에 그릴 것, 있으면 그것**. 얹을 데가 없는 조합(스티커의 색, 글자의 굵기)은
조용히 건너뛰고, 형광펜의 굵기는 저장된 배수(`HIGHLIGHT_WIDTH_FACTOR`)를 지킨다.

**신규 파일** `src/draw/transform.ts`(+테스트)·`e2e/draw3.spec.ts`. **기존 스펙은 한 건도
고치지 않았다** — 이번 회차의 e2e도 전부 additive다(단위 시험은 `drawSession.test.ts`의 걸음
모양 두 줄만 새 타입에 맞췄다). 패치노트는 **블록 2에서 v26 하나로** 묶는다(이번 회차 없음).

### M53-2 — 드로우 대개편 블록 2: 사진 요소·컬러 보드·종이·잠금 (2026-09-06)

M53의 **블록 2**(사진과 색). 블록 1이 깐 바닥(제스처 상태 머신·걸음 묶음·방어
`default`·`[data-draw-chrome]`) 위에 요소 타입 하나와 **additive optional 필드 넷**을
얹었다. 스키마는 여전히 1이고, 이 회차를 모르는 빌드에서 최악의 결말은 「사진 요소
하나가 안 보이는 것」이다(블록 1이 못박은 규칙 그대로).

**① 붙인 사진 = `DrawImage` 요소 (B2).** `{type:'image', x,y,w,h, photoId, opacity?}`.
바이트는 배경·카드 사진과 **완전히 같은 길**이다(`preparePhoto` → 새 id로
`putPhotoBlob` → 그다음에 메타데이터). 워크스페이스 JSON은 요소당 **140바이트**만
늘어난다(M53-fix 실측: 이미지 요소 50개에 1588B → 8586B, 요소당 140.0B — 좌표 넷·
`photoId`·`updatedAt`의 값이다) — **이것이 사진을 요소로 넣어도 안전한 이유의 전부**다. 배경(껍데기)은
그대로 두었다: 「종이」와 「붙인 사진」은 다른 물건이고, 껍데기를 요소로 옮기면 병합
규칙이 바뀐다(M52b의 경고는 여전히 유효하다). 그 대신 §6-15의 불평(「배경은 한 장이고
못 옮긴다」)은 이 요소가 통째로 해소한다.

입력은 셋인데 **문은 하나**다(`insertImage`): 도구 바의 「사진」(label+hidden input),
캔버스 드롭(`dragover`에서 막지 않으면 브라우저가 그 파일을 탭에서 연다), 그리고
붙여넣기. 붙여넣기는 이제 편집기가 열려 있는 동안 언제나 듣고 **배경 시트가 열려
있으면 배경, 아니면 요소**로 갈린다. `clipboardData.files`만 보던 M52b의 코드에
**`items[].getAsFile()` 폴백**을 더했다 — 스크린샷 붙여넣기를 `items`에만 담는
브라우저에서 아무 일도 일어나지 않던 자리다. 놓이는 크기는 **보이는 화면의
60%**(`DRAW_IMAGE_FIT`)이고 자리는 드롭 지점(없으면 뷰 중앙)이다.

**GC 목숨줄 두 곳이 이 회차의 진짜 위험이었다.** `utils/photos.referencedPhotoIds`와
`sync/exportImport.referencedIds`가 배경만 보고 있었다 — 요소 루프를 빠뜨리면 붙인
사진이 **30초 뒤에 사라지고, 화면에는 남아 있다가 새로고침에서 깨진다**(가장 나쁜
실패 모양). 두 곳 다 요소를 훑고, **지운 요소도 센다**: 요소의 삭제는 도장일 뿐이라
30일 동안 되살아날 수 있고(실행취소·상대의 병합), 그 사이에 바이트가 쓸려 가면
되살아난 것은 깨진 사진이다.

**② 렌더는 부모가 맵을 내려 준다.** `DrawElementView`가 훅 없는 순수 프레젠테이션이라는
규칙(그래야 draft 미리보기와 PNG가 같은 함수를 쓴다)을 지키려고, 새 훅
`stores/photoBlobs.usePhotoUrls(ids)`가 `photoId→url` 맵을 만들어 prop으로 내려간다.
요소마다 훅을 하나씩 두면 획 300개짜리 페이지가 리렌더마다 구독 300개를 만든다.

**③ PNG의 특수 경우가 사라졌다.** `svgToPngBlob`의 `backgroundDataUrl?: string`이
`imageDataUrls?: Record<Id,string>`이 되고, 복제본의 **`[data-photo-id]`가 달린 것
전부**가 같은 규칙으로 바이트를 갈아 끼운다(맵에 없는 id는 지운다 — blob: URL은
떨어져 나온 SVG 안에서 막힌다). 배경도 그 속성을 달아서 이제 「그 맵의 한 항목」이다.
경계(`exportBounds`)는 `elementBounds`에 `case 'image'`를 더한 것만으로 자동으로
사진을 감싼다.

**④ 컬러 보드 (C).** 도구 바의 여섯은 그대로 남고(근육 기억), 그 뒤의 팔레트 버튼이
색 시트를 연다: `DRAW_PALETTE` **41색**(12색조×3명도 + 무채색 5, 여섯 색과 값이
같다) · 최근 색 8(`drawSession`의 모듈 메모리) · `<input type="color">`.
`normalizeHex`(순수·시험 대상)가 소문자 `#rrggbb`로 정규화하는 이유는 대소문자 두
벌이면 「최근 색」이 같은 색을 둘로 세기 때문이다. 획의 색과 **채우기 색이 같은
시트**를 쓴다.

**⑤ 채우기·선 스타일 (#2·#4).** `DrawBox.fill`은 M52a부터 모델과 `hitTest`에 이미
있었고 UI만 없었다 — 「스타일」 시트가 그 손잡이다. `dash?: boolean`(사각형·타원·직선·
화살표)과 `heads?: 'end'|'both'`(화살표)는 additive optional이라 옛 빌드가 조용히
무시한다. 촉은 **점선이 되지 않는다**(끊긴 촉은 촉으로 안 보인다), 두 번째 촉은
`arrowHead`의 방향만 뒤집은 같은 함수다. 기본값은 **필드를 만들지 않는다** — 없는 것이
곧 실선이라 저장된 모양이 이 회차 이전과 같다.

**⑥ 종이와 스냅 (#5).** `DrawPage.paper?: 'plain'|'grid'|'dot'`은 제목·배경과 **같은
층의 껍데기**(`putPageShell`)라 평범한 LWW로 갈리고 두 사람에게 같이 보인다.
`'plain'`은 필드를 지운다. 무늬는 SVG `pattern`이고 색이 CSS 변수가 아니라 날 값인
이유는 `png.ts`의 그것과 같다 — 그래서 PNG에도 그대로 나간다. 무늬 한 칸은 격자
넷(32px)이다(8px 무늬는 4000×4000에서 잿빛이 된다). **스냅은 반대로 세션 서랍**의
것이다: 종이는 페이지의 성질이고 스냅은 이 기기의 손버릇이다. 스냅은 **움직인 거리**에
걸린다(요소의 좌표에 걸면 격자에서 벗어나 있던 그림이 첫 픽셀에 튄다). 손글씨는
붙이지 않는다 — 붙이면 손글씨가 아니게 된다.

**⑦ 손글씨 스무딩 (#7).** `strokePath`(Catmull-Rom → 3차 베지에)가 `polyline`을 대신
그린다. **렌더만 바뀐다** — 저장·히트 판정·PNG가 보는 값은 점 배열 그대로라 이미 그린
그림에도 소급된다. 대신 비용이 붙었다: 300획×60점의 path 문자열이 한 프레임에 9.6ms다
(실측, 폰은 그 몇 배). 그래서 `DrawElementView`를 **`memo`로 감쌌다** — 이 컴포넌트가
훅도 이벤트도 없는 순수 프레젠테이션이고 `previewElement`가 손대지 않은 요소는 **같은
객체**를 돌려주므로 얕은 비교가 정확히 성립한다. 그리는 중 매 프레임 300개를 다시
그리던 것이 이제 바뀐 것만 그린다.

**⑧ 잠금 (#10).** `locked?: boolean` additive. 잠긴 요소는 `pickTopElement`·
`marqueeHits`·이동·리사이즈·삭제·스타일에서 전부 빠진다 — **예외는
`pickTopElement(..., includeLocked)` 한 자리**이고, 그 예외가 없으면 잠근 것을 다시 풀
길이 없다. 선택은 「고른 것」과 「움직일 것」으로 갈렸다(`selection` / `movable`):
잠긴 것을 선택 자체에서 빼면 팝오버의 잠금 버튼이 뜰 자리가 없다.

> **그 예외는 이제 둘이다 (M53-fix ②).** 이 회차는 예외를 **Shift+클릭 하나**에만 달았고,
> 그래서 Shift가 없는 기기에서는 한 번 잠근 것을 영영 풀 수 없었다(테스터 2기 확정,
> `lock-*.png`). 이제 **손가락으로 탭하면** 그 자리에 「🔒 잠김 — 풀기」 한 줄이 뜬다
> (`draw-locked-hint` — 마우스에는 안 뜬다: Shift+클릭이라는 길이 이미 있고, 팝오버를 띄우면
> 다음 클릭이 그 위에 떨어진다). 요소는 여전히 안 잡힌다 — 잠금이 하는 일이 그것이므로.

**⑨ 지우개 크기 (#11).** 새 상태를 만들지 않았다 — **굵기 선택을 지우개에서 다시 읽는
것**이다(`eraserRadius`: 2→6px, 4→14px, 8→28px). 「굵게」로 그리다 지우개로 바꾸면 굵게
지워지는 것이 손이 기대하는 일이고, 도구마다 상태를 따로 두면 그 기대가 깨진다.

**신규 파일** `e2e/draw4.spec.ts`(12건). **기존 스펙 변경 둘**: 단위 시험 두 곳의
「모르는 타입」 이름이 `'image'`에서 `'card'`로 옮겨 갔고(진짜 타입이 됐으니), `draw3`의
선택자 한 줄이 `polyline` → `path, polyline`이 됐다(손글씨가 곡선이 됐으니). 그 밖의
스펙은 한 건도 고치지 않았다. 패치노트는 **v26**으로 블록 1과 블록 2를 함께 싣는다.

### M53-fix — 테스터 3기가 확정한 결함 다섯 (2026-09-06)

블록 1·2가 얹힌 그대로의 작업 트리 위에서, 세 갈래 검수(T1 데스크톱 정확성 · T2 폰 터치 ·
T3 동시 편집·GC·문서)가 **재현 스크립트로 확정한** 결함 다섯을 고쳤다. 모델 변경 **0**,
기존 스펙 변경 **0** — 이번 회차도 전부 additive다.

**① 새 페이지가 4000×4000의 왼쪽 위 구석에서 열렸다** (T1 `t8-edge`·`t9-view-probe`).
기전이 얄궂다: 뷰를 서랍에 적는 이펙트가 **첫 커밋에서** `{0,0,1}`을 적었고(그때 캔버스의
크기는 아직 0×0이다), 그 다음 크기가 잡힌 뒤에 도는 중앙 정렬 이펙트가 `rememberedView(page.id)
?? 가운데`에서 방금 적힌 그 오염값을 **「지난 방문의 자리」로** 읽었다. 첫 페이지도, 그 다음
페이지도 영영 구석에서 열린다. 고침은 뷰에 **`settled` 한 칸**을 더한 것이다 — 「자리 잡은
뷰인가」. 자리채움 뷰는 적지 않고, 화면도 그 값 대신 가운데를 본다. 가운데의 산술은 이제
순수 함수 하나다(`tools.centeredView`, 시험 3건): 페이지의 절반에서 화면의 절반을 뺀 자리.

**② 폰에서 잠근 것을 풀 수 없었다** (T2 `lock-*.png`). M53-2 ⑧의 해제 경로가 Shift+클릭
하나뿐이라, Shift가 없는 기기에서 잠금은 **되돌릴 수 없는 상태**였다. 잠긴 것을 손가락으로
탭하면(끌면 여전히 마퀴다) 그 자리에 「🔒 잠김 — 풀기」 한 줄이 뜬다. 요소는 **여전히 안
잡힌다** — 잠금이 하는 일이 그것이므로. 「풀기」는 한 걸음이고 풀린 것은 곧바로 손에
잡혀 있다(다음에 할 일이 그것이다). 이 한 줄은 **손가락일 때만** 뜬다: 마우스에는 이미
Shift+클릭이 있고, 팝오버를 띄우면 그 다음 클릭이 팝오버 위에 떨어진다.

**③ 글자 도구로 캔버스 위쪽을 탭하면 시트가 곧바로 닫혔다** (T2 `ghost-*.png`). 사람에게는
「아무 반응이 없다」로 보이던 자리다. pointerdown이 시트를 여는 순간, 브라우저가 그 뒤에
쏘는 **호환용 click**이 방금 마운트된 `sheet-overlay`(=닫기) 위에 떨어졌다. 두 겹으로
막는다: 텍스트 케이스의 `event.preventDefault()`가 그 합성 click 자체를 없애고, `Sheet`의
오버레이는 **마운트 뒤 300ms 안의 click을 무시**한다(여는 손짓의 뒤끝은 열림 애니메이션
240ms보다 늦지 않고, 사람이 밖을 눌러 닫는 일은 그보다 한참 뒤다 — 「밖을 누르면 닫힌다」는
그대로 산다). 두 번째 겹은 모든 시트가 함께 받는다.

**④ 데스크톱 도구 바의 「확대(+)」가 잘렸다** (T1 `toolbar-1280.png`). 둘째 줄 내용이
`max-w-3xl` 상자보다 넓었다 — 실측 **792px 대 752px**(폰·태블릿 크기에서는 864px). 상자를
768px부터 창 폭에 맡기고(`md:max-w-none`), 데스크톱에서는 다시 좁힌 대신 둘째 줄이
**접히게** 했다(`lg:flex-wrap` — 그 대역의 도구 바는 화면 아래 띠가 아니라 문서 안의
줄이라 키가 늘어도 아무것도 가리지 않는다). 폰(<768px)은 예전 그대로 **가로 스크롤**이다:
390px에서 접으면 도구 바가 화면의 절반을 먹는다. 실측 잘림 0 — 1280·1024·900.

**⑤ 붙인 사진의 진하기 (T1 관찰 ②).** 배경에만 있던 손잡이를 요소에도 줬다. 문은 **같은
문**이다(`clampOpacity`, 0.2~1) — 아래 한계가 0인 슬라이더는 「사진이 사라졌다」로 끝난다.
슬라이더는 「스타일」 시트에 있고, 사진을 고르면 뜨는 팝오버의 「진하기」가 그 문이다.
`applyStyle`을 그대로 쓰지 않은 이유 하나: 그것은 부를 때마다 실행취소 한 걸음을 남기고,
슬라이더는 한 번 끄는 동안 수십 번 값을 바꾼다. 시작할 때의 모습을 ref에 접어 두고 손을
뗄 때 **한 걸음**만 남긴다(획·리사이즈와 같은 규칙: 저장은 손을 떼는 그 한 번).

**신규 파일** `e2e/draw5.spec.ts`(5건 — 데스크톱 셋 + Pixel 5 둘). ②·③은 **폰
컨텍스트에서만 재현된다**(§6-10): 데스크톱 컨텍스트의 뷰포트를 줄이는 것으로는 합성 click도
터치 탭도 일어나지 않는다. 단위는 `centeredView` 3건이 늘었다. 새 testid 여섯
(`draw-locked-hint`·`draw-locked-unlock`·`draw-image-opacity`·`draw-image-opacity-open`·
`draw-image-opacity-value`·`draw-toolbar-styles`)은 전부 **추가**다. **기존 스펙은 한 건도 고치지 않았다.**
패치노트는 v26에 네 줄이 더 붙었고(가운데에서 열림·폰 잠금 풀기·글자 탭·사진 진하기·확대
버튼), 잠금 줄의 문구가 「Shift+클릭」에서 「폰은 탭, 컴퓨터는 Shift+클릭」으로 바뀌었다.

### M54 — 드로우의 기본이 「손」이 되고, 가운데로 돌아오는 문 하나 (2026-09-06)

사용자의 말 두 마디에서 나온 회차다. ① 「어디에 있든 페이지 가운데로 가고 배율을 1로
되돌리는 버튼」 ② 「기본은 아무것도 그리지 않는 손(이동) 모드 — 폰에서 손가락으로 화면을
옮길 때 그림이 그려지면 안 된다」.

**기본 도구가 손이 됐다.** `DRAW_TOOLS`의 첫 칸이 손이고 `defaultTools().tool`도 `'hand'`다.
폰에서 M53까지의 첫 경험은 「옆으로 넘기려다 그었다」였고, 그러면 사람은 그리기를 배우기
전에 되돌리기를 먼저 배워야 한다. 옮기기는 각오 없이 하는 일, 그리기는 마음먹고 하는 일 —
기본은 아무것도 남기지 않는 쪽이다. 손 도구의 동작 자체는 M52a 그대로다(한 손가락 팬,
휠·두 손가락 줌, 획 없음). 숫자 단축키는 배열 순서를 그대로 읽으므로 **한 칸씩 밀렸다**
(1=손, 2=펜 … 4=지우개, 5=선택).

**「가운데」 버튼**(`draw-recenter`, 아이콘 `locate`, 키보드 `0`). 축소·확대와 **같은 span
안**에 서서 셋이 한 덩어리로 접힌다(M53-fix ④의 그 규칙). 자리 계산은 새 페이지가 열릴 때와
**같은 함수**(`tools.centeredView`)라 「처음 열었을 때와 같은 자리」라는 약속이 코드 한 곳에
있다. 서랍(`rememberView`)은 직접 건드리지 않는다 — 뷰가 바뀌면 적는 것은 아래의 effect
하나뿐이고, 길이 둘이 되면 어느 쪽이 마지막인지 아무도 모른다. `0`은 도구 번호가 1부터라
비어 있던 키이고, 입력 중일 때는 물러선다(`isTyping` — 다른 단축키와 같은 문).

**딸린 레이아웃 한 건.** 버튼 하나가 늘면서 도구 바 둘째 줄이 **864→912px**이 됐고, 태블릿
대역(768~1023px, 상자가 창을 다 쓰는 그 대역)에서 900px 기준 28px이 넘쳤다 — M53-fix ④가
못 박아 둔 자리다. 둘째 줄을 `lg:flex-wrap`에서 **`md:flex-wrap`**으로 내려 그 대역에서도
접히게 했고(폰은 예전 그대로 가로 스크롤 — 390px에서 접으면 도구 바가 화면의 절반을 먹는다),
접히면 키가 자라므로 캔버스 밑의 자리채움을 **박힌 7.5rem에서 잰 높이로** 바꿨다
(`ResizeObserver` 하나). 다음에 버튼이 또 붙어도 이 숫자를 고칠 사람은 없다.

**신규 파일** `e2e/draw6.spec.ts`(5건 — 데스크톱 넷 + Pixel 5 하나: 기본이 손이라 끌어도
요소가 0이고 뷰만 움직인다 / 두 번 확대+팬 뒤 버튼 한 번에 배율 1·한가운데 / 키보드 `0`도
같은 일 / 펜을 고르면 예전처럼 그려진다 / 폰에서 손가락 하나로 옆으로 밀어도 선이 없다).
새 testid는 `draw-recenter` 하나, **추가**다. 기존 스펙은 **최소로만** 고쳤다: 다섯 드로우
...
13. ~~M52b — 드로우 다음 회차~~ **끝났다 (2026-09-05)**: 배경 이미지·카드 연결·PNG
    내보내기 셋 다. 용량 상수는 여전히 `src/draw/limits.ts`의 `DRAW_WARN_BYTES`
    **한 줄**(8MB, 서버 10MB 벽의 80%)이다. 남은 것은 아래 15·16번이다.
14. **`photos.spec.ts:200`(사진 포함 백업 왕복)은 전체 스위트 부하에서만 늦는 스펙이다.** 단독 실행은 매번 14s 안에 끝나는데 259건 병렬 실행에서는 「가져왔어요」 알림 대기 15s가 세 번 모자랐다(M50~M51 게이트 4회 중 3회). M51에서 그 대기만 30s로 늘렸다 — 건너뜀이 아니라 기다림. 다시 빨개지면 부하가 아니라 진짜 회귀로 보고 단독 재실행부터. 같은 종류가 하나 더: `pwa.spec.ts:85`는 `navigator.serviceWorker.ready`가 **activating** 순간에도 풀려 부하에서 `activating`을 읽은 적이 있다(M52b 게이트) — statechange로 `activated`까지 기다리도록 견고화.
15. ~~드로우 배경은 페이지당 한 장이고 자리를 옮길 수 없다~~ **해소됐다 (M53-2)**:
    붙인 사진이 **요소**(`DrawImage`)가 되어 여러 장을 옮기고 키우고 겹칠 수 있다.
    배경(껍데기)은 **일부러 그대로 뒀다** — 「종이」와 「붙인 사진」은 다른 물건이고,
    껍데기를 요소로 옮기면 병합 규칙이 바뀐다(M52b의 경고는 여전히 유효하다). 남은
    한 줄: 배경은 여전히 페이지 가운데 고정이다.
16. **드로우 세션 상태(`stores/drawSession`)는 새로고침에서 사라진다** — 의도된 것이지만,
    「폰에서 실수로 새로고침했더니 확대가 풀렸다」는 불평이 나오면 뷰만 `uiPersist` 옆으로
    옮기면 된다(실행취소 스택은 그대로 두어야 한다 — 되돌릴 대상이 이미 상대의 손을 지났을
    수 있다).
17. **드로우 M53의 다음 회차 대기 넷 (M53-2에서 합의된 순서).** ① **템플릿
    페이지**(#9 — 새 페이지에 격자 띠·9칸을 요소로 미리 넣기, 모델 변경 0),
    ② **카드 미리보기 요소**(#12 — `type:'card'`, B2와 같은 하위호환 리스크와 같은
    완화책. 「없으면 흐리게」 렌더와 prop 맵까지 B2가 이미 길을 냈다), ③ **페이지
    썸네일**(#16 — 목록 렌더 비용 때문에 요소 상한 N개만 그리는 절충 필요),
    ④ **회전**(#20 — `elementBounds`의 축평행 가정이 깨지므로 bbox·히트·핸들·PNG가
    전부 회전을 알아야 한다. 넷 중 유일하게 L 크기다).
18. **「그림이 무거워졌다」의 실제 지표는 이제 사진 저장소다.** `DRAW_WARN_BYTES`
    (8MB, `src/draw/limits.ts` 한 줄)는 M53-2로 거의 움직이지 않는다 — 붙인 사진이
    요소당 `photoId` 하나(백 바이트 언저리)만 싣기 때문이다. 진짜 비용은 idb와
    `image.php` 쪽이고 그것은 카드 사진과 **같은 예산**이다. 다음 세션이 이 상수를
    「그림의 무게」로 잘못 읽지 않도록 여기 적어 둔다: 그림이 무거워지는 것은
    획 수(워크스페이스 JSON)와 사진 장수(사진 저장소) **두 축**이고, 경고 한 줄은
    앞의 것만 본다.
19. **사진 blob 저장소(`photoBlobs`, idb `trip-board-photos`)는 세션별로 나뉘지 않는다.**
    M53-fix의 T3가 실측했다: 세션 B에 머무는 동안 도는 GC가 **세션 A의 로컬 blob을 쓸어
    갈 수 있다**(GC는 「지금 워크스페이스가 참조하는 id」만 살리고, 그 워크스페이스는 지금
    세션의 것이다). 그래도 사진이 사라지지는 않는다 — 서버 사본은 `X-Session` 격리로 무사하고,
    세션 A로 돌아오면 `loadPhotoUrl`의 원격 폴백이 다시 받아 온다(**자가치유**). 그래서 이것은
    버그가 아니라 **설계 트레이드오프**로 적어 둔다: 저장소를 세션별로 쪼개면 같은 사진을 두
    번 들고 있게 되고, 오프라인에서 세션을 오간 직후에는 사진 몇 장이 잠깐 다시 받아진다.
    바꾸기로 한다면 그 대가는 idb 용량이다.
