 ...
 * 도구가 기본값으로, 실행취소 스택이 빈 채로 돌아왔다 — 그리던 사람에게 그건
...
/**
 * 서랍이 비었을 때의 손 — **도구는 「손(이동)」에서 시작한다** (M54).
 *
 * M53까지의 기본은 펜이었다. 폰에서 그 기본은 「화면을 옆으로 밀어 보자」는 첫
 * 손가락을 그대로 획으로 만들었고, 사람은 그리기를 배우기 전에 **되돌리기를**
 * 먼저 배워야 했다. 옮기기는 아무 각오 없이 하는 일이고 그리기는 마음먹고 하는
 * 일이니, 기본은 아무것도 남기지 않는 쪽이다 — 그리려면 펜을 고른다.
 *
 * 도구 바의 첫 칸도 같은 이유로 손이다(`draw/tools`의 `DRAW_TOOLS`). 이
 * 값은 저장되지 않으므로(이 파일의 규칙) 새로고침하면 다시 손으로 돌아온다.
 */
